...
fn main(){
    ...
    let k = (1.0 + 3.322 * f64::log10(n as f64)).ceil() as usize; // формула Стерджеса
    let R = get_random(a, b, n as i32);
    

    let mean_X = mean(&R);
    let mean_theory = (a + b) / 2.0;
    let dispersion_X = dispersion(&R);
    let dispersion_theory = (b - a).powi(2) / 12.0;
    let hi_sqr = hi_squared(&R, a, b,  k);
...
fn hi_squared(R: &Vec<f64>, a: f64, b: f64,  k: usize) -> f64 {
    ...
    for i in 0..k {
        let mut n_counts = 0;
        for x in R {
            if *x >= a + ((i)  as f64) * (b - a) / (k as f64) 
            && *x < a + ((i + 1) as f64) * (b - a) / (k as f64) {
                n_counts += 1;
            }
        }
